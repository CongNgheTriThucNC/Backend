...
const createJobRequirementSchema = Joi.object({
    ...
    requirementType: Joi.string().valid('Skill', 'Education', 'Experience').required().messages({
        'any.only': 'Requirement type must be either "Skill", "Education", or "Experience".',
    }),
});

export const createJobRequirementDtoValidator = (req: Request, res: Response, next: NextFunction) => {
    const { error } = createJobRequirementSchema.validate(req.body, { abortEarly: false });
